''' Compared to #19, it replaces the "long" lists, i.e.
the lists of n elements points and closest, with Numpy
arrays (1D)
'''
import random
import sys
...
arr_points  = np.array(points, dtype='float32')
...
centroids = []
for i in range(k):  
    centroids.append(random.uniform(points[0], arr_points[-1]))
...
while change > 1e-4:    #main loop of K-means iteration
    ...
    for i in range(n):
        distances = []  #leave this as list for now!
        for j in range(k):
